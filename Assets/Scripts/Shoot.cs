...
public class Shoot : MonoBehaviour
{
	...
	private void ShootProjectile()
	{
        ...
        Rigidbody projectileRigidBody = Instantiate(m_projectile, m_ShootPoint.transform.position, m_ShootPoint.rotation) as Rigidbody;
        projectileRigidBody.velocity = projectileRigidBody.transform.forward * m_InitialVelocity;

        //O podemos especificar en unity que dos layer no queremso que choquen, edit > projectSettings > physics
        Collider projectileColider = projectileRigidBody.GetComponent<Collider>();
        Collider myCollider = transform.root.GetComponent<Collider>();
        Physics.IgnoreCollision(projectileColider, myCollider);
    ...
    private void ShootRay()
    {
       ...
       if( Physics.Raycast(m_ShootPoint.position, m_ShootPoint.forward, out RaycastHit hitInfo, m_ShootRange))
        {
            Rigidbody rb = hitInfo.collider.GetComponent<Rigidbody>();
            if (rb != null)
            {
                rb.AddExplosionForce(m_ShootForce, hitInfo.point, 1.0f);
            }
